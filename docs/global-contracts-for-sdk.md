...
- Scope: Changes are confined to the SDK and wasm‑signer‑worker (Rust + TS), reusing the existing `SignTransactionsWithActions` flow. Contract code (`email-recoverer`) and CLI tooling are already handled elsewhere.
...
Although the wasm‑signer‑worker is the source of truth for NEAR actions, we need matching TS types so apps can request global‑contract actions.

Files:
- `sdk/src/core/types/actions.ts`
- `sdk/src/core/WebAuthnManager/LitComponents/common/tx-digest.ts`
- Any call sites that construct `ActionArgs` for deploy flows (e.g. `AuthService`, `rpcCalls.ts`, `TatchiPasskey` helpers).
